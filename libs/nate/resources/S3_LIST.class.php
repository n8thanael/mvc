...
class S3_LIST extends SX_LISTCHECK {
    ...
    public function wash($text) {
        ...
        if (strlen($text) > 0) {
            ...
            // Also if there are bullets present...deal with those
            if ($this->linum <= 4 && $this->brnum >= 3) {
                $remix = true;
                $text = str_ireplace('&bull;', '<br>', $text);
                $text = str_ireplace('&BULL;', '<br>', $text);
            ...
            if ($remix) {
                ...
                if (count($array) > 1) {
                    ...
                    // Lines with lots of capital letters move to the top of the array:
                    foreach ($sentencearray as $key => $value) {
                        $array_a[$key] = strlen($value);
                        $array_c[$key] = strlen(preg_replace('![^A-Z]+!', '', $value)) / strlen($value);
                    };
                    ...
                    if (isset($array_c)) {
                        ...
                        foreach ($array_c as $key => $value) {
                            if ($value > .30) {
                                $array_b[$key] = $sentencearray[$key];
                            } else {
                                $array_d[$key] = $sentencearray[$key];
                            }
                        }
                        ...
                        $array_b = array_merge($array_b, $array_d);

                        $sort = false;
                    }
                    // run a parent function to get the average length of the lines & maybe we'll resort them,
                    if ($this->averagelinecheck($array_a) < 100 && (!$bull_conversion) && ($sort)) {
                        arsort($array_a);
                    }
                    ...
                    $sentencearray = array_unique($array_b);
                } else {
